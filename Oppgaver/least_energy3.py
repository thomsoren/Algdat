...
def init_single_source_bfs(nodes, edges, s):
    for node in nodes.values():
        node.d = float('inf') 
        node.pi = [] 
    for u,v,_ in edges:
        nodes[v].pre.append(nodes[u])
        nodes[u].after.append(nodes[v])    
    s.d = 0

def get_nodes_from_edges(edges, start, goal):
    nodes = {}
    for u, v, _ in edges:
        if u not in nodes:
            nodes[u] = Node(u)
    ...
    if goal not in nodes:
        nodes[goal] = Node(goal)
    return nodes

def bellman_ford(edges, start, goal):
    nodes = get_nodes_from_edges(edges, start, goal)
...
def bfs(start_node, goal, edges):
    # dekomponere til strongly connected components
    # to dfs
    ...
    while queue:
        node = queue.popleft()
        # går igjennom alle edges, lagre adjesent
        for u, v, _ in edges:
...
def least_energy(reactions, start, goal, laws_of_thermodynamics=False):
    bool, nodes, edges = bellman_ford(reactions, start, goal)
    ...
    if bool:
        ...
        return bfs(start, goal, edges)
    
    #print([node.d for node in nodes.values()])
    #bool, nodes, edges = bellman_ford(edges, start, goal)
    #while not bool:
        #bool, nodes, edges = bellman_ford(edges, start, goal)
    # Construct the path after removing negative cycles
...
print(least_energy([('A', 'B', -1), ('B', 'C', -2), ('C', 'A', -3)], "A", "C"))
